...
As another example, if we want to find all the variables with for which the respondent was either the father or the mother, we can use the "in" operator like so:
...
By default, the `operation` parameter is set to equals, but it can be specified for wild card matching as in the above. Below is a list of valid operations:
...
* **is_null*: is null / missing (does not require a value)
* **is_not_null*: is not null / not missing (does not require a value)
