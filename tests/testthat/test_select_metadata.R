...
test_that("Select Metadata returns a data frame when default parameters used", {
  default_call <- select_metadata(variable_name = "ce3agefc")
  expect_that(default_call, is_a("matrix"))
})

test_that("Select Metadata returns a list when returnDataFrame set to False", {
  list_return <- select_metadata(variable_name = "ce3agefc", returnDataFrame = FALSE)
  expect_that(list_return, is_a("list"))
})

test_that("Select Metadata returns a character type when one field is specified", {
  one_field <- select_metadata(variable_name = "ce3agefc", fields = "data_type")
  expect_that(one_field, is_a("character"))
})

test_that("Select Metadata returns a data frame when multiple fields specified", {
